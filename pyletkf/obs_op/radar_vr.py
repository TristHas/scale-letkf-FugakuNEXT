...
from .radar_reflectivity import (
    RADAR_USE_MELT,
    USE_T08_RS2014,
    linear_reflectivity_method3,
)

EARTH_RADIUS = 6_371_000.0
USE_TERMINAL_VELOCITY = True
...
def compute_vr(obs_slice: torch.Tensor, los_vectors: torch.Tensor) -> torch.Tensor:
    ...
    if USE_TERMINAL_VELOCITY:
        vt = _compute_terminal_velocity(
            qr,
            qs,
            qg,
            temp,
...
def _compute_terminal_velocity(qr, qs, qg, temp, press):
