...
from .io import load_letkf_state, load_obsda_sorted, load_radar
...
def process_full_pipeline(tile_idx=0, device="cuda:1"):
    ...
    states = {
        pe: load_letkf_state(dump_dir, f"pe{pe:06d}", "anal_f", strip_hallow=True).sel(ens=list(MEMBERS))
        for pe in tqdm(range(1))
    }
    
    da_state = pre_letkf(obs, states, device)
    da_params = letkf_core(da_state[tile_idx])
    
    state = states[tile_idx]["state"].transpose("z", "y", "x", "ens", "variable").values.contiguous()
