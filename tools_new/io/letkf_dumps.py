...
def load_obsda(dump_dir: str | Path, pe_tag: str | int) -> xr.Dataset:
    ds = _load_obsda_dataset(dump_dir, "obsda_after_obsope", pe_tag)
    ds = ds.assign_coords(obs=ds.isel(member=0, drop=True)["idx"])
    ds = ds.sel(member=['mem0001', 'mem0002']).squeeze()
    return ds

def load_obsda_sorted(dump_dir: str | Path, pe_tag: str | int) -> xr.Dataset:
    ds = _load_obsda_dataset(dump_dir, "obsda_after_set_letkf", pe_tag)
    ds = ds.assign_coords(obs=ds.isel(member=0, drop=True)["idx"])
    ds = ds.isel(member=0, drop=True)
    ds["aux_dim_0"] = ['mem0001', 'mem0002']
    return ds.rename({"aux_dim_0": "member"})
